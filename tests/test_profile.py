...
script: str = os.path.abspath(f"{current_dir}/../libs/profile.sh")
...
def test_profile_read_config_file_not_exists(bash):
    with bash() as s:
        s.auto_return_code_error = False
        assert s.run_script(script, ['profile_read_config']) == "Profile configuration file not found!"
        assert s.last_return_code == 1
